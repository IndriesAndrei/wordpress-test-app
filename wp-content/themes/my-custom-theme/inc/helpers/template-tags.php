...
// get posted on time for Posts
function mycustomtheme_posted_on() {
    ...
    $posted_on = sprintf(
        esc_html_x('Posted on %s', 'post date', 'mycustomtheme'),
...
function mycustomtheme_posted_by() {
    $byline = sprintf(
        esc_html_x('by %s', 'post author', 'mycustomtheme'),
...
// excerpt of the post
function mycustomtheme_excerpt($trim_character_count = 0) {
